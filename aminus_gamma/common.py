...
def get_params(
    ...
    params = {
        ...
        # Regime
        "min_inter_stimuli_interval_ms": 300.0,
        "max_inter_stimuli_interval_ms": 500.0,
        # Reward
...
def build_model(name, params, reward_timesteps):
    ...
    # STDP initial state
    if params["aPlus_b"] == 0:
        aPlus_state = params["aPlus"]
    else:
        aPlus_state = genn_model.init_var(
            "Gamma", {"a": params["aPlus_a"], "b": params["aPlus_b"]}
        )

    if params["aMinus_b"] == 0:
        aMinus_state = params["aMinus"]
    else:
        aMinus_state = genn_model.init_var(
            "Gamma", {"a": params["aMinus_a"], "b": params["aMinus_b"]}
        )
